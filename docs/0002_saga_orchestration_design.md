...
- **파생 문서**: `docs/0003_sync_http_architecture.md`(A안) · `docs/backlog/0004_async_messaging_architecture.md`(B안, 보류)
...
- **결정 3** — 보상은 성공한 단계의 역순으로 돈다. 정방향 순서나 응답값 추론은 버렸다.
...
- **성공한 단계의 역순으로 돌고, 어디까지 성공했는지는 `order_saga`가 기록한다.**
- 역순으로 돌면 다른 주문이 곧바로 쓸 수 있는 포인트가 먼저 풀리고, 재고 복구가 실패해도 포인트는 이미 돌아가 있다. 대신 성공한 단계를 기록해 둬야 한다.
...
- **`PLACING` 상태로 배제하고 두 번째 요청은 `409 ORDER_LOCKED`를 받는다.**
...
- 각각의 시스템 아키텍처와 그 안에서 갈린 결정은 `docs/0003`(A안)과 `docs/0004`(B안)에 있다.
...
### 11.3 롤백 A — 재고 부족, 보상 0회

첫 단계가 실패하면 되돌릴 것이 없다. 보상이 필요 없는 유일한 실패 경로다.

![롤백 A 시퀀스. 재고 차감이 실패하면 보상 없이 주문만 FAILED로 바꾸고 409 INSUFFICIENT_STOCK을 반환한다](images/03-rollback-stock.svg)

### 11.4 롤백 B — 잔액 부족, 보상 1회

재고는 이미 줄어든 상태다. 재고 복구를 호출해 되돌린 뒤에야 클라이언트에게 실패를 알린다.

![롤백 B 시퀀스. 재고 차감 성공 후 포인트 차감이 실패하면 재고 복구를 한 번 호출해 되돌린 뒤 FAILED로 바꾸고 409 INSUFFICIENT_POINT를 반환한다](images/04-rollback-point.svg)

### 11.5 롤백 C — 결제 실패, 보상 2회

가장 비싼 경로다. 성공한 단계의 역순으로 포인트 환불과 재고 복구를 차례로 부른다.

![롤백 C 시퀀스. 재고와 포인트가 모두 차감된 뒤 결제가 실패하면 포인트 환불과 재고 복구 순으로 두 번 보상하고 FAILED로 바꾼 뒤 409 PAYMENT_FAILED를 반환한다](images/05-rollback-payment.svg)

### 11.6 실패 지점별 최종 상태

- **주문이 없으면** 아무 일도 일어나지 않고 `404 ORDER_NOT_FOUND`를 준다.
- **이미 `COMPLETED`면** 아무 일도 하지 않고 `200`을 준다.
- **사가가 진행 중인데 다시 요청하면** 주문은 `PLACING` 그대로이고 `409 ORDER_LOCKED`를 준다.
- **재고가 부족하면** 되돌릴 것이 없으므로 보상 없이 주문을 `FAILED`로 닫고 `409 INSUFFICIENT_STOCK`을 준다.
- **잔액이 부족하면** 재고를 복구하고 주문을 `FAILED`로 닫은 뒤 `409 INSUFFICIENT_POINT`를 준다.
- **포인트가 없으면** 마찬가지로 재고를 복구하고 `404 POINT_NOT_FOUND`를 준다.
- **결제가 실패하면** 포인트 환불과 재고 복구를 역순으로 하고 주문을 `FAILED`로 닫은 뒤 `409 PAYMENT_FAILED`를 준다.
- **보상 자체가 실패하면** 워커가 재시도하고 주문은 `PLACING`에 머문다. 사가는 `COMPENSATION_FAILED`가 되고 `500 INTERNAL_ERROR`와 함께 알림이 나간다.
...
요구사항 문서에 반영할 것은 `PLACING`·`FAILED` 상태와 `PAYMENT_FAILED`·`ALREADY_PAID` 코드, 그리고 결제가 더 이상 포인트 차감을 포함하지 않는다는 점이다.
구현 착수와 같은 작업 단위로 `docs/0001`에 반영한다. 이는 통신 방식과 무관하게 두 안 모두에 해당한다.

나머지 후속은 각 안의 문서가 갖는다. A안은 `docs/0003` §6, B안은 `docs/0004` §12다.
