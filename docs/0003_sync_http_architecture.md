...
- **A-1 사가 진행 상태는 단계마다 `order_saga`에 커밋한다.** 진행 상태가 호출 스택에만 있으면 프로세스가 죽었을 때 어디까지 갔는지 알 방법이 없다.
  이를 위해 네 서비스 모두 `spring.jpa.open-in-view: false`로 둔다.
  OSIV가 켜져 있으면 요청 동안 세션이 열려 있어 커밋한 뒤에도 커넥션이 반납되지 않고, 원격 호출 내내 커넥션을 쥔 채 이전 단계의 엔티티 상태를 다시 쓴다.
...
- **A-24 결제까지 성공한 `RUNNING` 사가는 워커가 정방향을 다시 돌리지 않고 `succeed`만 다시 시도한다.** 재실행 도중 참여자의 일시 장애가 보상으로 이어져, 이미 승인된 결제와 차감이 통째로 되돌려졌다.
  판별은 `claim`이 잠금 아래에서 읽은 `payment_done`으로 한다.
  `payment_done`은 결제가 `200`을 돌려준 뒤에만 켜지고 단계는 순서대로 실행되므로, 켜져 있으면 세 단계가 모두 성공했다.
  이 판별은 `order_saga`를 바꾸는 메서드가 모두 사가 행을 대기형 락으로 읽어야 성립한다.
  잠그지 않으면 전진 쪽이 보상 쪽의 `COMPENSATING`을 옛 `RUNNING`으로 덮어써, 결제가 취소된 사가가 완료로 닫힐 수 있다.
  락 순서는 `orders` → `order_saga`이고, `claim`만 `SKIP LOCKED`다.
  `succeed`가 실패하면 보상하지 않고 `attempts`와 `last_error`만 남긴다.
  3회째 실패에 알림을 한 번 보내고, 그 뒤에도 스윕마다 계속 시도한다.
  상한 뒤 자동 보상은 기각했다. 원인이 order DB면 보상도 같이 실패하고, 세 단계가 성공한 주문의 결제를 취소하는 것이 이 결정이 막으려는 일이다.
  `@Version` 낙관 락은 새 기제이고 충돌 때의 재시도 경로를 따로 설계해야 해서 기각했다.
  `payment_done`이 꺼진 사가는 결제 성공 여부를 모르므로 A-17대로 처음부터 재실행하고, 실패하면 보상한다.
...
- `RUNNING`이면서 `payment_done`이 켜져 있으면 `succeed`만 다시 시도하고, 실패해도 보상하지 않는다(A-24).
- 그 밖의 `RUNNING`이면 `forward()`를 재실행하고, 나머지는 보상을 다시 돌린다. 한 건이 터져도 나머지 처리를 막지 않는다.
- 실패하면 `updated_at`이 갱신되어 임계 뒤에 다시 집히므로, **스윕 반복 자체가 무한 재시도**가 된다.
- 워커마저 실패하면 `AlertSender`가 알림을 보내 사람이 개입한다. 지금 구현은 로그만 남기는 대역이다.
  알림은 두 종류다. 보상 실패는 실패할 때마다, 결제까지 끝난 사가의 완료 실패는 3회째에 한 번 보낸다.
