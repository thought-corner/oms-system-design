...
class AlertMessageFormatterTest : BehaviorSpec({
    ...
    Given("정방향 요청을 처리할 수 없는데 실패 응답도 쓰지 못한 알림") {
        When("메시지로 만들면") {
            ...
            Then("즉시 확인 머리말로 주문이 멈췄을 수 있고 어드민이 직접 정리해야 한다고 먼저 적는다") {
                message shouldStartWith "🔴 [즉시 확인] 주문 10의 재고 차감 요청을 처리할 수 없어 멈췄습니다"
                message shouldContain "자동 취소도 시작되지 못했습니다"
                message shouldContain "어드민 수동 처리가 필요합니다. 주문 상태와 재고를 직접 확인해"
            }
        }
    }
    ...
    Given("되돌리는 요청을 처리할 수 없는 알림") {
        When("메시지로 만들면") {
            val message = AlertMessageFormatter.format(POISON_UNREPLIED_ALERT.copy(messageType = "STOCK_CANCEL"))

            Then("되돌려지지 않은 채 남아 있을 수 있으니 수동으로 되돌리라고 적는다") {
                message shouldStartWith "🔴 [즉시 확인] 주문 10의 재고 복구 요청을 처리할 수 없어 멈췄습니다"
                message shouldContain "재고가 차감된 채 남아 있을 수 있습니다"
    ...
    Given("실패 응답을 쓴 정방향 요청의 알림") {
        When("메시지로 만들면") {
            ...
            Then("결함 머리말로 주문은 실패 처리되고 되돌릴 것이 없다고 적는다") {
                message shouldStartWith "🟡 [결함] 주문 10의 재고 차감 요청이 잘못되어 주문을 실패 처리했습니다"
    ...
    Given("정방향 요청이 재처리를 모두 실패한 알림") {
        When("메시지로 만들면") {
            ...
            Then("장애 머리말로 아직 바뀐 것이 없고 복구되면 이어진다고 적는다") {
                message shouldStartWith "🟠 [장애] 주문 10의 재고 차감이 시스템 장애로 멈췄습니다"
                message shouldContain "재고는 아직 차감되지 않았습니다"
    ...
    Given("되돌리는 요청이 재처리를 모두 실패한 알림") {
        When("메시지로 만들면") {
            val message = AlertMessageFormatter.format(RETRY_EXHAUSTED_ALERT.copy(messageType = "STOCK_CANCEL"))

            Then("되돌려지지 않은 채 남아 있을 수 있고 복구되면 자동으로 되돌려진다고 적는다") {
                message shouldStartWith "🟠 [장애] 주문 10의 재고 복구가 시스템 장애로 멈췄습니다"
                message shouldContain "재고가 차감된 채 남아 있을 수 있습니다"
                message shouldContain "재고는 자동으로 되돌려지므로"
            }
        }
    }
