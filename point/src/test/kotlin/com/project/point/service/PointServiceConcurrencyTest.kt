...
import com.project.point.fixture.newTransaction
...
class PointServiceConcurrencyTest : BehaviorSpec() {
    ...
    init {
        ...
        Given("포인트 사용이 가드를 쥔 채 아직 커밋하지 않은 사가") {
            ...
            val userId = 9001L
            val clock = PointFixture.FIXED_CLOCK
            ...
            transactionManager.newTransaction().execute { pointRepository.save(PointFixture.point(userId = userId, amount = 10000L, id = null)) }
            ...
            When("같은 사가의 보상이 도착하면") {
                val result = try {
                    ...
                    val canceller = executor.submit { transactionManager.newTransaction().execute { service.cancel(UseCancelCommand(sagaId, 1L)) } }
                    val waited = try {
                        Thread.sleep(500)
                        !canceller.isDone
                    } finally {
                    ...
                    val refunded = transactionManager.newTransaction().execute {
                        historyRepository.findBySagaIdAndTransactionType(sagaId, PointTransactionType.CANCEL)?.amount
                    }
                    val amount = transactionManager.newTransaction().execute { pointRepository.findWithLockByUserId(userId)?.amount }
                ...
                Then("보상은 가드에서 기다렸다가 커밋된 사용을 보고 사용량만큼 CANCEL 이력을 남기며 잔액을 되돌린다") {
