...
private fun pointService(
    ...
    guardRepository: SagaGuardRepository = PointFixture.guardRepository(),
    replyOutbox: SagaReplyOutbox = replyOutbox(),
): PointService = PointService(
    pointRepository,
    historyRepository,
    SagaGuardLock(guardRepository, PointFixture.FIXED_CLOCK),
    replyOutbox,
    PointFixture.FIXED_CLOCK,
)
...
class PointServiceTest : BehaviorSpec({
    ...
    Given("포인트가 없는 사용자 99") {
        ...
        When("포인트를 쓰면") {
            val exception = shouldThrow<BusinessException> { service.use(PointFixture.useCommand(userId = 99L, amount = 1L)) }
    ...
    Given("잔액 399인 사용자 1") {
        ...
        When("400을 쓰면") {
            val exception = shouldThrow<BusinessException> { service.use(PointFixture.useCommand()) }
    ...
    Given("잔액 10000인 사용자 1") {
        val pointRepository = mockk<PointRepository>()
        val historyRepository = mockk<PointTransactionHistoryRepository>()
        val guardRepository = PointFixture.guardRepository()
        val replyOutbox = replyOutbox()
        ...
        When("400을 쓰면") {
            service.use(PointFixture.useCommand())
            ...
            Then("사가 가드를 FORWARD로 잡은 뒤 이력을 읽고 행 락으로 잔액을 차감해 USE 이력과 성공 응답을 같은 트랜잭션에 남긴다") {
                ...
                verifyOrder {
                    ...
                    replyOutbox.succeeded(PointMessageType.POINT_USE, "saga-1", 1L)
                }
                verify(exactly = 0) { replyOutbox.failed(any(), any(), any(), any()) }
                verify(exactly = 0) { pointRepository.findById(any()) }
                verify(exactly = 1) {
                    historyRepository.save(
                        match<PointTransactionHistory> {
                            it.transactionType == PointTransactionType.USE && it.createdAt == PointFixture.FIXED_NOW
                        },
                    )
                }
            }
    ...
    Given("같은 sagaId로 이미 사용한 이력이 있는 사용자") {
        ...
        When("같은 sagaId로 다시 사용을 요청하면") {
            ...
            Then("잔액을 건드리지 않고 성공 응답을 다시 남긴다") {
                ...
                verify(exactly = 0) { historyRepository.save(any()) }
                verify(exactly = 1) { replyOutbox.succeeded(PointMessageType.POINT_USE, "saga-1", 1L) }
            }
        }
    }
    ...
    Given("보상이 먼저 도착해 CANCEL 가드가 남은 sagaId") {
        ...
        val historyRepository = mockk<PointTransactionHistoryRepository>()
        val replyOutbox = replyOutbox()
        val service = pointService(pointRepository, historyRepository, PointFixture.guardRepository(SagaGuardKind.CANCEL), replyOutbox)
    ...
    Given("사용 이력이 없는 sagaId") {
        ...
        When("보상을 요청하면") {
            service.cancel(UseCancelCommand(sagaId = "saga-none", orderId = 1L))

            Then("CANCEL 가드를 남기고 되돌릴 것 없이 실패 대신 성공 응답을 남긴다") {
                verify(exactly = 1) { replyOutbox.succeeded(PointMessageType.POINT_CANCEL, "saga-none", 1L) }
                verify(exactly = 0) { replyOutbox.failed(any(), any(), any(), any()) }
    ...
    Given("사용 이력이 있고 아직 되돌리지 않은 sagaId") {
        ...
        When("보상을 요청하면") {
            service.cancel(UseCancelCommand(sagaId = "saga-1", orderId = 1L))

            Then("잔액이 10000으로 돌아오고 사용 이력의 주문·금액으로 CANCEL 이력을 남긴다") {
                point.amount shouldBe 10000L
                verify(exactly = 1) { replyOutbox.succeeded(PointMessageType.POINT_CANCEL, "saga-1", 1L) }
                verify(exactly = 1) {
                    historyRepository.save(
                        match<PointTransactionHistory> {
                            it.transactionType == PointTransactionType.CANCEL && it.orderId == 5L && it.amount == 400L
                        },
                    )
                }
            }
    ...
    Given("이미 되돌린 sagaId") {
        ...
        When("같은 sagaId로 보상을 다시 요청하면") {
            ...
            Then("잔액을 두 번 되돌리지 않고 성공 응답만 다시 남긴다") {
                verify(exactly = 0) { pointRepository.findWithLockByUserId(any()) }
                verify(exactly = 0) { historyRepository.save(any()) }
                verify(exactly = 1) { replyOutbox.succeeded(PointMessageType.POINT_CANCEL, "saga-1", 1L) }
            }
    ...
    Given("잔액 부족으로 사용 트랜잭션이 롤백된 사가") {
        ...
        When("실패 응답을 기록하면") {
            service.recordUseFailure(PointFixture.useCommand(), PointErrorCode.INSUFFICIENT_POINT)

            Then("비즈니스 변경 없이 POINT_USE 실패 응답만 남긴다") {
                verify(exactly = 1) {
                    replyOutbox.failed(PointMessageType.POINT_USE, "saga-1", 1L, PointErrorCode.INSUFFICIENT_POINT)
                }
                verify(exactly = 0) { replyOutbox.succeeded(any(), any(), any()) }
            }
        }
    }
})
