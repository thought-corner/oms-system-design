...
class OutboxRelayBrokerOutageIntegrationTest : BehaviorSpec() {
    ...
    init {
        ...
        Given("브로커가 멈춘 동안 order 스키마에 쌓인 두 토픽의 미발행 행") {
            ...
            pauseBroker()
            tables.insert(OutboxSource.ORDER, IntegrationTestConfig.COMMAND_TOPIC, sagaIds[0], held = true)
            tables.insert(OutboxSource.ORDER, IntegrationTestConfig.REPLY_TOPIC, sagaIds[1], held = true)

            When("임대가 실패 한도보다 많이 끝나 릴레이가 매번 다시 집어 보내도") {
                repeat(OutboxRelayPolicy.MAX_PUBLISH_ATTEMPTS + 2) {
                    tables.release(OutboxSource.ORDER, sagaIds)
                    eventually(10.seconds) {
                        sagaIds.forEach { tables.claimedAt(OutboxSource.ORDER, it).shouldNotBeNull() }
                    }
                ...
                Then("브로커에 닿지 않은 배치라 실패를 세지 않아 fail_count 0 인 PENDING 으로 남고 FAILED 알림도 없다") {
                    sagaIds.forEach { sagaId ->
                        val pending = tables.row(OutboxSource.ORDER, sagaId)
            ...
            When("브로커가 돌아온 뒤 임대가 끝나면") {
                ...
                Then("같은 행이 FAILED 에 이르지 않고 발행되어 PUBLISHED 가 된다") {
                    eventually(75.seconds) {
                        sagaIds.forEach { tables.row(OutboxSource.ORDER, it)["status"] shouldBe "PUBLISHED" }
                    }
                    sagaIds.forEach { (tables.row(OutboxSource.ORDER, it)["fail_count"] as Int) shouldBeLessThan OutboxRelayPolicy.MAX_PUBLISH_ATTEMPTS }
                    verify(exactly = 0) { alertSender.send(any<OutboxPublishFailedAlert>()) }
                }
            }
        }
