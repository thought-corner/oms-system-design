...
class SagaGuardRepositoryTest : BehaviorSpec() {
    ...
    private fun lockGuard(sagaId: String, kind: SagaGuardKind) {
        guardRepository.insertIfAbsent(sagaId, kind.name, PaymentFixture.FIXED_PAID_AT)
    ...
    init {
        ...
        Given("가드가 없는 sagaId") {
            ...
            When("FORWARD로 넣은 뒤 같은 sagaId를 CANCEL로 다시 넣으면") {
                guardRepository.insertIfAbsent("saga-guard-1", SagaGuardKind.FORWARD.name, PaymentFixture.FIXED_PAID_AT)
                guardRepository.insertIfAbsent("saga-guard-1", SagaGuardKind.CANCEL.name, PaymentFixture.FIXED_PAID_AT)
        ...
        Given("다른 트랜잭션이 이미 있는 가드를 잡고 있는 sagaId") {
            ...
            newTransaction().execute { guardRepository.insertIfAbsent(sagaId, SagaGuardKind.FORWARD.name, PaymentFixture.FIXED_PAID_AT) }
            ...
            When("같은 sagaId의 보상이 가드를 잡으려 하면") {
                val holder = executor.submit {
                    newTransaction().execute {
                        guardRepository.insertIfAbsent(sagaId, SagaGuardKind.FORWARD.name, PaymentFixture.FIXED_PAID_AT)
                        locked.countDown()
